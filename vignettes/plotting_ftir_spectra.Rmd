...
All of these shifts are achieved by calling the `shift_baseline()` function.
...
biodiesel |>
  shift_baseline(method = "average", wavenumber_range = c(2000, 1900), individually = TRUE) |>
  plot_ftir() |>
  zoom_in_on_range(c(2000, 1000))
```
...
biodiesel |>
  shift_baseline(method = "point", wavenumber_range = 1250, individually = TRUE) |>
  plot_ftir() |>
  zoom_in_on_range(c(2000, 1000))
```
...
biodiesel |>
  shift_baseline(method = "minimum", wavenumber_range = c(1300, 1000), individually = FALSE) |>
  plot_ftir() |>
  zoom_in_on_range(c(2000, 1000))
```
...
Tous ces changements sont réalisés en appelant la fonction `shift_baseline()`.
