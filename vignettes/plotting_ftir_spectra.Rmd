...
```{r basicplot_en, fig.cap="A number of plotted FTIR spectra of multiple samples overlaid."}
plot_ftir(spectra)
```
...
```{r offsetplot_en, fig.cap="A number of plotted FTIR spectra, each offset slightly from the others."}
...
```{r titles_en, fig.cap="The first overlaid spectral plot, with title 'Title of my Spectra Plot'."}
...
```{r rename_samples_en, fig.cap="The first overlaid spectral plot, with two samples renamed."}
plot_ftir(spectra) |>
...
```{r compress_en, fig.cap="The first overlaid spectral plot with the lower-energy region compressed."}
plot_ftir(spectra) |>
...
```{r r_zoom_en, fig.cap="The first overlaid spectral plot, zoomed in to the 3600 to 2600 wavenumbers region."}
plot_ftir(spectra) |>
...
```{r markers_en, fig.cap="The first spectral plot with a marker at 1495 cm-1, indicating 'C-C Aromatic'."}
plot_ftir(spectra) |>
...
```{r combo_en, fig.cap="A complex spectral plot"}
...
```{r transmittance_en, fig.cap="The initial spectral plot with transmittance units on the y axis."}
...
```{r scalar_en, fig.cap="The initial spectral plot with two components shifted up 0.2 absorbance units."}
...
```{r plot_biodiesel_en, fig.cap="A spectral plot of biodiesel samples zoomed in to the 2000 to 1000 wavenumber range."}
plot_ftir(biodiesel) |>
...
```{r plot_biodiesel_baseline_1_en, fig.cap="The above plot with baselines recalculated to align spectra at 0."}
biodiesel |>
...
```{r plot_biodiesel_baseline_2_en, fig.cap="The above plot, but with the spectra set to a 0 value at exactly 1250 cm-1."}
biodiesel |>
...
```{r plot_biodiesel_baseline_3_en, fig.cap="The above plot, with spectra shifted to the minimal value between 1300 and 1000 wavenumbers set to 0."}
biodiesel |>
...
```{r basicplot_fr, fig.cap="Un certain nombre de spectres FTIR superposés de plusieurs échantillons."}
...
```{r offsetplot_fr, fig.cap="Un certain nombre de spectres FTIR tracés, chacun légèrement décalé par rapport aux autres."}
...
```{r titles_fr, fig.cap="Le premier graphique spectral superposé, intitulé 'Titre de mon tracé de spectres'."}
...
```{r rename_samples_fr, fig.cap="Le premier graphique spectral superposé, avec deux échantillons renommés."}
plot_ftir(spectres, lang = "fr") |>
  rename_plot_sample_ids(sample_ids = c("methylbenzene" = "toluene", "C7" = "heptanes"))
```
...
```{r compress_fr, fig.cap="Le premier graphique spectral superposé avec la région de basse énergie comprimée."}
plot_ftir(spectres, lang = "fr") |>
  compress_low_energy(cutoff = 1850, compression_ratio = 5)
```
...
```{r r_zoom_fr, fig.cap="Le premier graphique spectral superposé, zoomé sur la région des nombres d'onde de 3600 à 2600."}
plot_ftir(spectres, lang = "fr") |>
  zoom_in_on_range(c(3600, 2600))
```
...
```{r markers_fr, fig.cap="Le premier graphique spectral avec un marqueur à 1495 cm-1, indiquant 'C-C aromatique'."}
plot_ftir(spectres, lang = "fr") |>
  add_wavenumber_marker(
    wavenumber = 1495,
    text = "C-C aromatique",
...
```{r combo_fr, fig.cap="Un diagramme spectral complexe"}
...
```{r transmittance_fr, fig.cap="Le diagramme spectral initial avec les unités de transmittance sur l'axe des y."}
...
```{r scalar_fr, fig.cap="Le diagramme spectral initial avec deux composantes décalées de 0.2 unité d'absorbance vers le haut."}
...
```{r plot_biodiesel_fr, fig.cap="Un graphique spectral d'échantillons de biodiesel zoomé sur la plage de nombres d'onde de 2000 à 1000."}
...
```{r plot_biodiesel_baseline_1_fr, fig.cap="Le graphique ci-dessus avec les lignes de base recalculées pour aligner les spectres à 0."}
biodiesel |>
  recalculate_baseline(method = "average", wavenumber_range = c(2000, 1900), individually = TRUE) |>
  plot_ftir(lang = "fr") |>
  zoom_in_on_range(c(2000, 1000))
...
```{r plot_biodiesel_baseline_2_fr, fig.cap="Le graphique ci-dessus, mais avec les spectres fixés à une valeur 0 à exactement 1250 cm-1."}
biodiesel |>
  recalculate_baseline(method = "point", wavenumber_range = 1250, individually = TRUE) |>
  plot_ftir(lang = "fr") |>
  zoom_in_on_range(c(2000, 1000))
...
```{r plot_biodiesel_baseline_3_fr, fig.cap="Le graphique ci-dessus, avec les spectres décalés à la valeur minimale entre 1300 et 1000 nombres d'ondes fixés à 0."}
biodiesel |>
  recalculate_baseline(method = "minimum", wavenumber_range = c(1300, 1000), individually = FALSE) |>
  plot_ftir(lang = "fr") |>
  zoom_in_on_range(c(2000, 1000))
