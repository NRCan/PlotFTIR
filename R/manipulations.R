...
#'  creation.
#'
#'  #' Cette fonction permet de renommer les ID d'échantillon comme indiqué dans
#'  la légende d'un tracé. Bien qu'il soit généralement préférable d'attribuer
#'  des noms propres dans les données, cette fonction permet de modifier les
#'  noms après la création du tracé.
#'
#' @param ftir_spectra_plot A plot generated by [plot_ftir()] or
#'  [plot_ftir_stacked()].
#'
#'  Un tracé généré par [plot_ftir()] ou [plot_ftir_stacked()].
#'
#' @param sample_ids A named vector of format `"old name" = "new name"`. Must include
#'  *all* sample ID old names (a pair of `"old name" = "old name"` is permissable
#'  and results in no renaming for that sample).
#'
#'  Un vecteur nommé au format `"ancien nom" = "nouveau nom"`. Doit inclure *tous*
#'  les anciens noms d'ID d'échantillon (une paire de `"ancien nom" = "ancien
#'  nom"` est autorisée et n'entraîne aucun changement de nom pour cet
#'  échantillon).
#'
#' @return the FTIR plot as a ggplot2 object, with samples renamed in the legend.
#'
#'  le tracé FTIR en tant qu'objet ggplot2, avec des échantillons renommés dans
#'  la légende.
#'
#' @export
#'
...
rename_plot_sample_ids <- function(ftir_spectra_plot, sample_ids) {
  ...
  if (!(all(names(sample_ids) %in% preexisting_sampleids))) {
    cli::cli_abort("All provided {.arg sample_ids} 'old names' must be in the {.arg ftir_spectra_plot}.")
  }
...
#' changed from vertical to horizontal. By default, legends are positioned with
#' `position = "right"`, `justification = "center"`, and `direction = "vertical"`.
#' For more complex legend manipulation please perform using
#' [ggplot::theme()] controls directly on the plot object. More information at
...
#' légende du tracé peut être déplacée vers différents emplacements, justifiée à
#' ces emplacements et/ou modifiée de verticale à horizontale. Par défaut, les
#' légendes sont positionnées avec `position = "right"`, `justification = "center"`
#' et `direction = "vertical"`. Pour une manipulation de légende plus
#' complexe, veuillez utiliser les contrôles [ggplot::theme()] directement sur
#' l'objet de tracé. Plus d'informations sur
#' [https://ggplot2.tidyverse.org/reference/theme.html](https://ggplot2.tidyverse.org/reference/theme.html#examples).
#'
#' @param ftir_spectra_plot A plot generated by [plot_ftir()] or
#'   [plot_ftir_stacked()].
...
#'
#' @param legend_title_position A position for the legend title relative to the legend items. One of `"top"`, `"right"`, `"left"`, or `"bottom"`.
#'
...
#'
#'   le tracé FTIR en tant qu'objet ggplot2, avec la légende déplacée si
#'   nécessaire
#'
#' @export
#'
