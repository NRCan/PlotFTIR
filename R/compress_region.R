...
#' @keywords internal
compress_trans <- function(intercept, reducer) {
  ...
  scales::trans_new(name = 'custom',
                    transform = trans,
                    inverse = inv)
}

rev_trans <- function(){
  scales::trans_new(name = "reverse",
...
compress_rev_trans <- function(intercept, reducer){
  trans <- function(x, i = intercept, r = reducer) {
    ifelse(x > i, -x, -(x / r + i))
  }
  inv <- function(x, i = intercept, r = reducer) {
...
compress_low_energy <- function(ftir_spectra_plot, cutoff = 2000, compression_ratio = 2) {
  ...
  p <- ftir_spectra_plot +
    ...
    ggplot2::coord_trans(x = compress_trans(intercept = -cutoff, reducer = compression_ratio))
