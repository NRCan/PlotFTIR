    ...
    .pkg_abort(
      list(
        ...
        fr = cli::format_inline(
          "Erreur dans {.fn PlotFTIR::get_plot_sample_ids}. {.arg ftir_spectra_plot} doit \u00e9tre un objet ggplot. Vous avez fourni {.obj_type_friendly {ftir_spectra_plot}}."
        )
      )
    )
  }
  ...
  if (!(is.data.frame(ftir))) {
    .pkg_abort(
      list(
        ...
        fr = cli::format_inline(
          "Erreur dans {.fn {fn}}. {.arg ftir} doit \u00e9tre un data.frame. Vous avez fourni {.obj_type_friendly ftir}."
        )
      )
    )
  }
